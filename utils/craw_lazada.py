...
def crawl_url(url, run_headless=False):
	...
	try:
		f = open('./ecommerce_prices/lazada_items_list.txt', 'wb')
		...
		categories = soup.findAll('li', attrs={'class':'sub-item-remove-arrow'})
		
		#'''
		for category in categories:
			category = category.find('a', href=True)
			category_link = "https:" + category['href']
			#print (category_link)
			...
			for page_number in range(1, n_pages):
				...
				items = category_soup.findAll('div', attrs={'class':"cRjKsc"})
