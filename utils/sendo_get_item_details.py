...
def crawl(urls, part_number):
	...
	fw = open('./ecommerce_prices/{}/sendo_items_details_{}.txt'.format(today, part_number), 'wb')
	fe = open('./ecommerce_prices/{}/sendo_items_error_{}.txt'.format(today, part_number), 'wb')	
	...
	for url in urls:
		try:
			...
			name      = soup.find('h1', attrs={'class':'productName_3Cdc'}).text
			print (price, name)
...
def crawl_url():
	...
	try:
		fr = open('./ecommerce_prices/sendo_items_list.txt', 'r')
		urls = fr.readlines()
